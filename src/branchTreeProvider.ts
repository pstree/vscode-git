...
export class BranchItem extends vscode.TreeItem {
    constructor(
        ...
        const baseContext = isHead ? 'localBranchCurrent' : itemContextValue;
        // Local branches without an upstream have never been pushed: tag the
        // contextValue (…NoUpstream) so the tree shows a push icon instead of the
        // pull icon, and so reset/pull are disabled for them.
        const branch = itemContextValue === 'localBranch' ? (ref as Branch) : undefined;
        const hasUpstream = !!branch?.upstream;
        const contextWithUpstream = (itemContextValue === 'localBranch' && !hasUpstream)
            ? `${baseContext}NoUpstream`
            : baseContext;
        ...
        if (itemContextValue === 'localBranch' && !hasUpstream) {
            this.description = '↑ no upstream';
            this.tooltip = `${ref.name} — not pushed (no upstream). Click the ↑ button to push and set upstream.`;
        }

        // Clicking (or double-clicking) a branch opens its Git History — same
        // action as the inline "View History" button. VS Code has no native
        // tree double-click event, so binding the item's command makes any
        // activation of the row trigger the history view.
        if (itemContextValue === 'localBranch' || itemContextValue === 'remoteBranch') {
            this.command = {
                command: 'gitBranches.openHistory',
                title: 'View History',
                arguments: [this],
            };
        }
...
export class BranchesProvider extends AbstractProvider {
    ...
    private loadingBranches = new Set<string>();
    ...
    // Track which branch is currently being fetched so only that branch's inline
    // refresh icon spins (instead of every branch). Keyed by repo path plus name
    // so identically-named branches in different repos don't collide. Toggling
    ...
    /**
     * Mark every branch (local + remote tracking refs) across the given repos as
     * loading so their inline refresh icons all spin during a bulk operation
     * like "Update All". Fires a single re-render if any key's state changed.
     */
    async setAllBranchesLoading(repos: Repository[], loading: boolean): Promise<void> {
        let changed = false;
        for (const repo of repos) {
            try {
                const locals = await repo.getBranches({ remote: false });
                for (const b of locals) {
                    if (b.name) { changed = this.setKeyLoading(repo, b.name, loading) || changed; }
                }
            } catch { /* repo unavailable */ }
            try {
                const remotes = await repo.getBranches({ remote: true });
                for (const b of remotes) {
                    if (b.name && !isHeadRef(b)) { changed = this.setKeyLoading(repo, b.name, loading) || changed; }
                }
            } catch { /* repo unavailable */ }
        }
    ...
    private async getLocalBranches(repo: Repository): Promise<BranchItem[]> {
        ...
        // repo.getBranches() does NOT reliably populate the `upstream` field, so
        // derive it ourselves with a single for-each-ref call (covers every local
        // branch, including the checked-out one).
        const upstreams = await this.getUpstreamMap(repo);
        return branches
            .sort((a, b) => {
            ...
            .map(r => {
                const loading = this.isBranchLoading(repo, r.name ?? '');
                ...
                const upstreamName = upstreams.get(r.name ?? '');
                let enriched: Branch = upstreamName
                    ? { ...r, upstream: parseUpstream(upstreamName) }
                    : r;
                if (r.name === headName && head) {
                    enriched = {
                        ...
                        ahead: head.ahead,
                        behind: head.behind,
                        // Prefer the git extension's own upstream for HEAD, then ours.
                        upstream: head.upstream ?? enriched.upstream,
                    };
                }
                return new BranchItem(enriched, repo, 'localBranch', undefined, undefined, loading);
            });
    }

    // Map of local branch name → upstream short name (e.g. "origin/main"), via git.
    private async getUpstreamMap(repo: Repository): Promise<Map<string, string>> {
        ...
        try {
            ...
            for (const line of stdout.trim().split('\n').filter(Boolean)) {
                ...
                const upstream = sep >= 0 ? line.slice(sep + 1) : '';
                if (name && upstream) { map.set(name, upstream); }
            }
        } catch { /* no branches / git unavailable */ }
        return map;
    }
    ...
    private async getRemoteBranches(repo: Repository, remoteName: string): Promise<BranchItem[]> {
        ...
        return cached
            ...
            .map(b => new BranchItem(b, repo, 'remoteBranch', shortName(b, remoteName), undefined, this.isBranchLoading(repo, b.name ?? '')));
    }
}
...
// Parse a short upstream ref (e.g. "origin/main") into the { name, remote } shape
// expected by the Branch type.
function parseUpstream(short: string): { name: string; remote: string } {
    const idx = short.indexOf('/');
    const remote = idx >= 0 ? short.slice(0, idx) : short;
    return { name: short, remote };
}
